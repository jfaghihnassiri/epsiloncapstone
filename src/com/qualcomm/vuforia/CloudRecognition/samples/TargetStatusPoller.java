...
//import com.qualcomm.vuforia.CloudRecognition.utils.SignatureBuilder; COMMENTED
...
public class TargetStatusPoller {
    ...
    class PollerTask extends TimerTask {
        public void run() {
			...
			} catch ( URISyntaxException e) {//( URISyntaxException | IOException e) {
				timer.cancel();
				e.printStackTrace();
				return;
			}
        	catch ( IOException e) {//( URISyntaxException | IOException e) {
				timer.cancel();
				e.printStackTrace();
				return;
			}
	...
	private JSONObject getTarget() throws URISyntaxException, ClientProtocolException, IOException {
		...
		if( response != null ){
			...
			} catch (JSONException e) {//(ParseException | JSONException e) {
				e.printStackTrace();
			}
